...
public class GameTestsAspectsRead {
    ...
    public void testAspectsReadRedstoneBooleanClock(GameTestHelper helper) {
        helper.setBlock(POS.west(), Blocks.REDSTONE_BLOCK);
        testReadAspect(helper, PartTypes.REDSTONE_READER, Aspects.Read.Redstone.BOOLEAN_CLOCK, ValueTypeBoolean.ValueBoolean.of(true));
    }
    ...
    public static void testReadAspect(GameTestHelper helper, IPartType<?, ?> partType, IAspectRead<?, ?> aspectRead, IValue expectedValue) {
        ...
        helper.succeedWhen(() -> {
            helper.assertTrue(partReader.getReadAspects().size() > 0, "Read aspect list is empty");
            IAspectVariable variable = partReader.getVariable(PartTarget.fromCenter(partPos), partStateReader, aspectRead);
            helper.assertTrue(variable != null, "Variable is null");
            try {
                GameTestHelpersIntegratedDynamics.assertValueEqual(variable.getValue(), expectedValue);
