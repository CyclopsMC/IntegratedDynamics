...
public class TestBlockOperators {
    ...
    public void testBlockTag() throws EvaluationException {
        ...
        TestHelpers.assertEqual(((ValueTypeList.ValueList) res1).getRawValue().getLength(), 14, "size(tag(sand)) = 14");

        IValue res2 = Operators.OBJECT_BLOCK_TAG.evaluate(new IVariable[]{bLeaves});
        TestHelpers.assertEqual(((ValueTypeList.ValueList) res2).getRawValue().getLength(), 5, "size(tag(leaves)) = 5");
    }
