 ...
 */
public abstract class IngredientChannelAdapter<T, M> implements IIngredientComponentStorageSlotted<T, M> {
    ...
    // ----- Expose slots by chaining storages sequentially, not using the index -----
    ...
    public int getSlots() {
        int slots = 0;
        IPositionedAddonsNetworkIngredients<T, M> network = getNetwork();
