...
public class GameTestsAspectVariables {
    ...
    protected static ContainerAspectSettings openAspectSettings(GameTestHelper helper, PartPos partPos, IAspect<?, ?> aspect) {
        ...
        if (!(menu instanceof ContainerAspectSettings container)) {
            throw new GameTestAssertException(Component.literal("The aspect settings container could not be created"), (int) helper.getTick());
        }
        return container;
    }
    ...
    protected static ContainerMultipartAspects openPartGui(GameTestHelper helper, PartPos partPos, IPartType<?, ?> partType) {
        ...
        AbstractContainerMenu menu = partType.getContainerProvider(partPos)
                .orElseThrow(() -> new GameTestAssertException(Component.literal("The part has no gui container"), (int) helper.getTick()))
                .createMenu(1, player.getInventory(), player);
        if (!(menu instanceof ContainerMultipartAspects container)) {
            throw new GameTestAssertException(Component.literal("The part gui container could not be created"), (int) helper.getTick());
        }
        return container;
    }
    ...
    protected static ContainerMultipartAspects openPartGuiFromNetwork(GameTestHelper helper, PartPos partPos, IPartType<?, ?> partType) {
        ...
        if (!(menu instanceof ContainerMultipartAspects container)) {
            throw new GameTestAssertException(Component.literal("The client-side part gui container could not be created"), (int) helper.getTick());
        }
        return container;
    }
    ...
    protected static ContainerMultipartAspects openPartGuiWriterFromNetwork(GameTestHelper helper, PartPos partPos, IPartType<?, ?> partType) {
        ...
        if (!(menu instanceof ContainerMultipartAspects container)) {
            throw new GameTestAssertException(Component.literal("The client-side writer part gui container could not be created"), (int) helper.getTick());
        }
        return container;
    }
     ...
     */
    protected static List<Component> syncPropertyValues(GameTestHelper helper, ContainerMultipartAspects container, IAspect aspect) {
    ...
    public void testAspectVariablesAbsentUsesDefaultProperty(GameTestHelper helper) {
        ...
        helper.succeedWhen(() -> assertValueEqual(helper, variableSupplier.get(),
    ...
    public void testAspectVariablesConstantOverridesStaticProperty(GameTestHelper helper) {
        ...
        setAspectPropertyVariable(partPos(helper, POS), Aspects.Read.Inventory.OBJECT_ITEM_STACK_SLOT,
                ...
                createVariableForValue(helper.getLevel(), ValueTypes.INTEGER, ValueTypeInteger.ValueInteger.of(2)));
        helper.succeedWhen(() -> {
            assertValueEqual(helper, variableSupplier.get(),
                    ValueObjectTypeItemStack.ValueItemStack.of(new ItemStack(Items.DIAMOND, 3)));
            ...
            assertValueEqual(helper, partState.getAspectProperties(Aspects.Read.Inventory.OBJECT_ITEM_STACK_SLOT)
    ...
    public void testAspectVariablesRemoval(GameTestHelper helper) {
        ...
        helper.startSequence()
                .thenWaitUntil(() -> assertValueEqual(helper, variableSupplier.get(),
                        ValueObjectTypeItemStack.ValueItemStack.of(new ItemStack(Items.DIAMOND, 3))))
    ...
    public void testAspectVariablesDynamic(GameTestHelper helper) {
        ...
        ChestBlockEntity driverChest = helper.getBlockEntity(POS.north(), ChestBlockEntity.class);
    ...
    public void testAspectVariablesDisplayedValueFollowsVariable(GameTestHelper helper) {
        ...
        helper.startSequence()
                ...
                .thenExecute(() -> assertValueEqual(helper, container.getEffectivePropertyValue(0), ValueTypeInteger.ValueInteger.of(1)))
                ...
                .thenExecute(() -> setAspectPropertyVariable(partPos, Aspects.Read.Inventory.OBJECT_ITEM_STACK_SLOT,
                        AspectReadBuilders.Inventory.PROPERTY_SLOTID,
                        createVariableForValue(helper.getLevel(), ValueTypes.INTEGER, ValueTypeInteger.ValueInteger.of(2))))
                .thenWaitUntil(() -> assertValueEqual(helper, container.getEffectivePropertyValue(0), ValueTypeInteger.ValueInteger.of(2)))
                // After removing the variable, the statically configured value is shown again
                .thenExecute(() -> setAspectPropertyVariable(partPos, Aspects.Read.Inventory.OBJECT_ITEM_STACK_SLOT,
                        AspectReadBuilders.Inventory.PROPERTY_SLOTID, ItemStack.EMPTY))
                .thenWaitUntil(() -> assertValueEqual(helper, container.getEffectivePropertyValue(0), ValueTypeInteger.ValueInteger.of(1)))
                .thenSucceed();
    }
    ...
    public void testAspectVariablesDisplayedValueFollowsDynamicVariable(GameTestHelper helper) {
        ...
        helper.startSequence()
                .thenWaitUntil(() -> assertValueEqual(helper, container.getEffectivePropertyValue(0), ValueTypeInteger.ValueInteger.of(0)))
                .thenExecute(() -> {
                    driverChest.setItem(0, new ItemStack(Items.STICK));
                    driverChest.setItem(1, new ItemStack(Items.STICK));
                    driverChest.setChanged();
    ...
    public void testAspectVariablesPartGuiPropertyValuesSync(GameTestHelper helper) {
        ...
        helper.succeedWhen(() -> {
            List<Component> synced = syncPropertyValues(helper, container, Aspects.Read.Inventory.OBJECT_ITEM_STACK_SLOT);
    ...
    public void testAspectVariablesPartGuiPropertyValuesSyncMultiple(GameTestHelper helper) {
        ...
        helper.succeedWhen(() -> {
            List<Component> synced = syncPropertyValues(helper, container, Aspects.Write.Redstone.BOOLEAN_PULSE);
            helper.assertTrue(synced != null, "The property values were not synced");
        ...
        helper.succeedWhen(() -> {
            ...
            for (Object aspectObject : container.getAspectPropertyButtons().keySet()) {
                ...
                try {
                    ...
                    throw new GameTestAssertException(Component.literal("Determining the property values of aspect "
                            + aspect.getUniqueName() + " failed: " + e), (int) helper.getTick());
                }
            }
        });
    }
    ...
    public void testAspectVariablesPartGuiBroadcastSyncsVariableDrivenValue(GameTestHelper helper) {
        ...
        helper.succeedWhen(() -> {
            ...
            if (values == null) {
                throw new GameTestAssertException(Component.literal("The property values of the strong power setting were not synced"), (int) helper.getTick());
            }
    ...
    @GameTest(template = TEMPLATE_EMPTY, environment = "integrateddynamics:aspectvariablesredstonepulse")
    ...
    @GameTest(template = TEMPLATE_EMPTY, environment = "integrateddynamics:aspectvariablesredstonepulselength")
    ...
    public void testAspectVariablesPersistence(GameTestHelper helper) {
        ...
        helper.succeedWhen(() -> {
            ...
            ItemStack partItem = partType.getItemStack(context, new PartPathElement(helper.absolutePos(POS)), partStateHolder.getState(), true);
            IPartState restoredState = (IPartState) partType.getState(context, new PartPathElement(helper.absolutePos(POS)), partItem);
    ...
    public void testAspectVariablesErrorPersistence(GameTestHelper helper) {
        ...
        helper.succeedWhen(() -> {
            ...
            IPartState restoredState;
            try (ProblemReporter.ScopedCollector scopedCollector = new ProblemReporter.ScopedCollector(
                    new PartPathElement(helper.absolutePos(POS)), LOGGER)) {
                TagValueOutput valueOutput = TagValueOutput.createWithContext(scopedCollector, context.holderLookupProvider());
                partType.serializeState(valueOutput, partStateHolder.getState());
                restoredState = (IPartState) partType.deserializeState(TagValueInput.create(
                        scopedCollector, context.holderLookupProvider(), valueOutput.buildResult()));
            }
    ...
    public void testAspectVariablesDropOnBreak(GameTestHelper helper) {
        ...
        helper.getBlockState(POS.above()).onDestroyedByPlayer(helper.getLevel(), helper.absolutePos(POS.above()), player, toolStack, true,
    ...
    public void testAspectVariablesViaContainer(GameTestHelper helper) {
        ...
        helper.startSequence()
                ...
                .thenWaitUntil(() -> assertValueEqual(helper, variableSupplier.get(),
                        ValueObjectTypeItemStack.ValueItemStack.of(new ItemStack(Items.DIAMOND, 3))))
                .thenExecute(() -> assertValueEqual(helper,
    ...
    public void testAspectVariablesUnaffectedAspects(GameTestHelper helper) {
        ...
        setAspectPropertyVariable(partPos(helper, POS), Aspects.Read.Inventory.OBJECT_ITEM_STACK_SLOT,
                AspectReadBuilders.Inventory.PROPERTY_SLOTID,
                createVariableForValue(helper.getLevel(), ValueTypes.INTEGER, ValueTypeInteger.ValueInteger.of(2)));
        helper.succeedWhen(() -> assertValueEqual(helper, variableSupplier.get(), ValueTypeInteger.ValueInteger.of(3)));
    }
    ...
    public void testAspectVariablesEffectiveProperty(GameTestHelper helper) {
        ...
        setAspectPropertyVariable(partPos, Aspects.Read.Inventory.OBJECT_ITEM_STACK_SLOT,
                ...
                createVariableForValue(helper.getLevel(), ValueTypes.INTEGER, ValueTypeInteger.ValueInteger.of(2)));
        helper.succeedWhen(() -> {
            assertValueEqual(helper, getEffectiveAspectProperty(partPos, Aspects.Read.Inventory.OBJECT_ITEM_STACK_SLOT,
