...
public class AspectAppendixClient extends SectionAppendixClient<AspectAppendix> {
    ...
    protected void drawElement(ScreenInfoBook gui, GuiGraphics guiGraphics, int x, int y, int width, int height, int page, int mx, int my) {
        ...
        guiGraphics.drawString(gui.getFont(), IModHelpers.get().getL10NHelpers().localize(aspect instanceof IAspectWrite ? L10NValues.GUI_INPUT : L10NValues.GUI_OUTPUT, valueString), x, y + 16, ARGB.opaque(0), false);

        // Settings
        if (aspect.hasProperties()) {
            int offsetY = 26;
            guiGraphics.drawString(gui.getFont(), ChatFormatting.DARK_GRAY + IModHelpers.get().getL10NHelpers().localize("gui.integrateddynamics.part.properties"), x, y + offsetY, ARGB.opaque(0), false);
            for (IAspectPropertyTypeInstance property : ((IAspect<?, ?>) aspect).getPropertyTypes()) {
                offsetY += 10;
                guiGraphics.drawString(gui.getFont(), ChatFormatting.DARK_GRAY + IModHelpers.get().getL10NHelpers().localize(property.getTranslationKey()), x + 10, y + offsetY, ARGB.opaque(0), false);
            }
        }
